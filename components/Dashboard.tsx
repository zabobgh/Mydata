...
const Dashboard: React.FC<DashboardProps> = ({ user, onLogout, appIcon, onUpdateAppIcon }) => {
  ...
  const renderView = () => {
    ...
    switch (activeView) {
      ...
      case 'approvals':
         return adminOnlyView(<ApprovalView requests={disbursementRequests} onApprove={(id) => handleApprovalAction(id, DisbursementStatus.Approved)} onReject={(id) => handleApprovalAction(id, DisbursementStatus.Rejected)} />);
      case 'audit_trail':
